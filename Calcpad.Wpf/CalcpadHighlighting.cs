...
{
    /// <summary>
    /// Syntax highlighting for Calcpad in AvalonEdit
    /// Supports Calcpad syntax + external language blocks
    /// </summary>
    public class CalcpadHighlighter : DocumentColorizingTransformer
    {
        // Calcpad colors
        ...
        // Calcpad keywords
        private static readonly HashSet<string> CalcpadKeywords = new HashSet<string>
        {
        ...
        // Calcpad functions (sample - add more as needed)
        private static readonly HashSet<string> CalcpadFunctions = new HashSet<string>
        {
        ...
        protected override void ColorizeLine(DocumentLine line)
        {
            ...
            // Check if line starts with # (Calcpad directive like #if, #for, etc.)
            if (trimmed.StartsWith("#"))
            {
                ChangeLinePart(lineOffset, lineOffset + line.Length, element => element.TextRunProperties.SetForegroundBrush(DirectiveBrush));
                return;
            ...
            // Check if line starts with $ (Calcpad commands like $svg, $end, $plot, $map)
            if (trimmed.StartsWith("$"))
            {
                ChangeLinePart(lineOffset, lineOffset + line.Length, element => element.TextRunProperties.SetForegroundBrush(DirectiveBrush));
                return;
            ...
            foreach (Match match in wordRegex.Matches(lineText))
            {
                ...
                else if (CalcpadFunctions.Contains(word))
                {
                    ChangeLinePart(
                        lineOffset + match.Index,
                        lineOffset + match.Index + match.Length,
