/*
 * DEPRECATED: This file has been moved to Calcpad.Common.Zip
 * The WPF project now uses the unified implementation from Calcpad.Common.
 * This file is kept for reference only.
 *
 * To use the common implementation, add: using Calcpad.Common;
 * Then use: Zip.Compress(), Zip.Decompress(), etc.
 *
 * Note: WPF-specific SpanLineEnumerator usage can be achieved with:
 *   Zip.Decompress(stream).AsSpan().EnumerateLines()
 *
...
// Temporary bridge class until MainWindow.xaml.cs is fully migrated
// This provides backward compatibility with existing WPF code
using System;
using System.IO;
using System.Text;
...
namespace Calcpad.Wpf
{
    ...
    {
        internal static void Compress(string text, Stream fs) =>
            Calcpad.Common.Zip.Compress(text, fs);

        internal static void CompressWithImages(string text, string[] images, string fileName) =>
            Calcpad.Common.Zip.CompressWithImages(text, images, fileName);

        internal static SpanLineEnumerator Decompress(Stream fs) =>
            Calcpad.Common.Zip.Decompress(fs).AsSpan().EnumerateLines();

        internal static string DecompressToString(Stream fs) =>
            Calcpad.Common.Zip.Decompress(fs);

        internal static bool IsComposite(string fileName) =>
            Calcpad.Common.Zip.IsComposite(fileName);

        internal static string DecompressWithImages(string fileName) =>
            Calcpad.Common.Zip.DecompressWithImages(fileName);
    }
}
