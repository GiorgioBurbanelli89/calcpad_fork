    ...
    {   
        ...
        static bool TryConvertOnStartup(Settings settings)
        {
            ...
            try
            {
                ...
                // If MultLang processed pure external code (no Calcpad math),
                // the result is already HTML - don't parse it with ExpressionParser
                List<string> openXmlExpressions = null;
                if (result.MultilangProcessed)
                {
                    // unwrappedCode is already HTML from MultLangProcessor
                    htmlResult = unwrappedCode;
                    openXmlExpressions = new List<string>();
                }
                else
                {
