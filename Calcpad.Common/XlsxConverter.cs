// XlsxConverter.cs - Conversor de Excel (.xlsx) a Calcpad (.cpd)
...
{
    /// <summary>
    /// Conversor de archivos Excel (.xlsx) a Calcpad (.cpd)
    /// </summary>
    public class XlsxConverter
    {
        ...
        private void ConvertSheet(ZipArchive archive, string sheetFile, string sheetName)
        {
            ...
            using (var stream = entry.Open())
            {
                ...
                // Generar código Calcpad limpio y compacto

                // 1. Sección de DATOS (constantes) - solo las que tienen label
                // Nota sobre keywords de Calcpad:
                ...
                if (labeledConstants.Count > 0)
                {
                    ...
                    foreach (var (varName, value, unit, label) in labeledConstants)
                    {
                        ...
                        // Sintaxis Calcpad: valor + unidad (pegados)
                        // Ejemplo: h = 25cm (asigna 25 centímetros)
                        var calcpadUnit = ConvertToCalcpadUnit(unit);
        ...
        private string ConvertExcelFormula(string formula, Dictionary<string, string> cellValues, Dictionary<string, string> cellLabels)
        {
            ...
            // Convertir funciones de Excel a Calcpad
            ...
            // Convertir IF con strings a comentarios (Calcpad no soporta strings en IF)
            ...
            // Convertir operadores de comparación de Excel a Calcpad
        ...
        private string ConvertToVariableName(string input)
        {
            ...
            // Reemplazar guiones bajos al inicio (Calcpad no permite _ al inicio)
        ...
        /// <summary>
        /// Convierte unidad de Excel a formato Calcpad
        /// </summary>
        private string ConvertToCalcpadUnit(string unit)
        {
            if (string.IsNullOrEmpty(unit)) return null;

            var u = unit.Trim();

            // Mapa de unidades comunes de Excel a Calcpad
        ...
        private string ConvertMinMaxFunctions(string input, Dictionary<string, string> cellValues, Dictionary<string, string> cellLabels)
        {
            ...
            do
            {
                ...
                result = Regex.Replace(result, @"(min|max)\(([^()]+)\)", m => {
                    ...
                    // Convertir : a ; (rango Excel a separador Calcpad)
                    // Convertir , a ; (separador Excel a Calcpad)
