...
<body>
    ...
    <script>
        ...
        /// This HTML is embedded directly in the Calcpad output and uses Virtual Host URLs
        /// Accepts either:
        ///   1) Path to IFC file
...
(function() {{
    ...
    initViewer();
        ...
        /// <summary>
        /// Generate HTML fragment for embedding in Calcpad output - works with autorun
        ...
        public static string GenerateFileBasedViewerWithCdn(string ifcFileName, string displayName)
        {
            ...
            // Generate HTML fragment (not full page) for embedding in Calcpad output
            return $@"<style>
...
<body>
    ...
    <script type=""module"">
        ...
        private static string GenerateViewerHtml(string viewerId, string ifcBase64, string fileName, string wasmBasePath)
        {
            ...
            {
                // Use CDN
                threeJsSrc = "https://cdn.jsdelivr.net/npm/three@0.170.0/build/three.min.js";
                orbitControlsSrc = "https://cdn.jsdelivr.net/npm/three@0.170.0/examples/js/controls/OrbitControls.js";
                webIfcSrc = "https://cdn.jsdelivr.net/npm/web-ifc@0.0.57/web-ifc-api-iife.js";
                locateFileLogic = "if (path.endsWith('.wasm')) { return 'https://cdn.jsdelivr.net/npm/web-ifc@0.0.57/' + path; }";
            }
            else
            {
                ...
                orbitControlsSrc = $"{wasmBasePath}/OrbitControls.js";
                webIfcSrc = $"{wasmBasePath}/web-ifc-api-iife.js";
                locateFileLogic = $"if (path.endsWith('.wasm')) {{ return '{wasmBasePath}/' + path; }}";
...
<script>
    (async function() {{
        ...
        try {{
            ...
            // Load IFC
            const ifcAPI = new WebIFC.IfcAPI();
            await ifcAPI.Init((p) => {{ {locateFileLogic} return p; }});
...
(function() {
    ...
    (async function() {{
        ...
        /// Ignores lines that start with ' (Calcpad comment syntax)
        /// </summary>
        private static string ParseDirective(string content, string directiveName)
        {
            // Remove commented lines (Calcpad uses ' for comments)
        ...
        /// Supports: // (C-style), # (shell/Python), ' (Calcpad - for compatibility)
        ...
        private static string RemoveCommentedLines(string content)
        {
            ...
            foreach (var line in lines)
            {
                ...
                // // = C-style, # = shell/Python, ' = Calcpad (legacy compatibility)
