...
namespace Calcpad.Common
{
    ...
    public class McdxConverter
    {
        ...
        private string ProcessMatrix(XElement matrix)
        {
            ...
            // En Calcpad: [v1; v2; v3] (solo punto y coma)
            if (cols == 1)
            {
                sb.Append(string.Join("; ", values));
            }
            else
            {
                ...
                // Calcpad: filas separadas por |, columnas por ;
        ...
        private string ProcessApply(XElement apply)
        {
            ...
            switch (op)
            {
                ...
                case "scale":
                    ...
                    // En Calcpad, las unidades se aplican con ': valor'unidad
                    if (children.Count >= 3)
                    {
                        ...
                        {
                            // Es una unidad - usar sintaxis Calcpad: valor'unidad
        ...
        private void GeneratePythonPlot(PlotInfo info)
        {
            ...
            _output.AppendLine();
            _output.AppendLine($"# Datos de Calcpad (convertir vectores a arrays numpy)");
            _output.AppendLine($"# NOTA: Las variables {info.XVariable} y {info.YVariable} deben estar definidas arriba");
            _output.AppendLine($"# Si son vectores de Calcpad, extraer valores manualmente:");
            _output.AppendLine();
            ...
            _output.AppendLine($"# Definir los datos (ajustar según las variables de Calcpad)");
        ...
        private void ProcessSolveBlockRegion(XElement solveblock, XNamespace ns)
        {
            try
            {
                ...
                _output.AppendLine("' (Solve blocks no soportados en Calcpad)");
                _output.AppendLine("' ==================================");
                _output.AppendLine();
            }
            catch (Exception ex)
        ...
        private string ProcessDefineWithUnit(XElement define)
        {
            ...
            if (valueElement.Name.LocalName == "apply")
            {
                ...
                if (applyChildren.Count >= 3 && applyChildren[0].Name.LocalName == "scale")
                {
                    ...
                    {
                        value = ProcessMatrixForCalcpad(matrixOrVector);
                    }
            ...
            // Generar expresión con unidad en formato Calcpad: [1;2;3]'m
        ...
        /// <summary>
        /// Procesa una matriz para Calcpad con formato [v1; v2; v3] para vectores columna
        /// En Calcpad:
        ...
        /// </summary>
        private string ProcessMatrixForCalcpad(XElement matrix)
        {
            var rowsAttr = matrix.Attribute("rows");
            var colsAttr = matrix.Attribute("cols");
            ...
            else
            {
                ...
                // Calcpad: [row0_col0; row0_col1; ... | row1_col0; row1_col1; ...]
                for (int r = 0; r < rows; r++)
                {
                    if (r > 0) sb.Append(" | ");
