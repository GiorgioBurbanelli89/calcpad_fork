        ...
        private void Parse(ReadOnlySpan<char> code, bool calculate, bool getXml)
        {
            ...
            try
            {
                while (++_currentLine < lineCount)
                {
                    ...
                    {
                        // Need to get the actual line text to check for $svg/$end
                        // before using cached tokens
                        var cachedLineStart = lines[_currentLine];
                        var cachedLineEnd = lines[_currentLine + 1];
                        var cachedLineText = code[cachedLineStart..cachedLineEnd].Trim();
                        var cachedLineString = cachedLineText.ToString(); // Convert for reliable comparison

                        // Skip cache processing for $svg and $end lines
                        bool isSvgRelated = cachedLineString.StartsWith("$s", StringComparison.OrdinalIgnoreCase) ||
                                           cachedLineString.StartsWith("$end", StringComparison.OrdinalIgnoreCase);

                        // Skip cache for SVG elements - detect by pattern regardless of _isSvgBlock state
                        // because _isSvgBlock may not be set yet if $svg wasn't processed
                        bool isSvgElement = cachedLineString.Contains('{') &&
                            (cachedLineString.StartsWith("line{", StringComparison.OrdinalIgnoreCase) ||
                             cachedLineString.StartsWith("rect{", StringComparison.OrdinalIgnoreCase) ||
                             cachedLineString.StartsWith("circle{", StringComparison.OrdinalIgnoreCase) ||
                             cachedLineString.StartsWith("ellipse{", StringComparison.OrdinalIgnoreCase) ||
                             cachedLineString.StartsWith("polygon{", StringComparison.OrdinalIgnoreCase) ||
                             cachedLineString.StartsWith("polyline{", StringComparison.OrdinalIgnoreCase) ||
                             cachedLineString.StartsWith("path{", StringComparison.OrdinalIgnoreCase) ||
                             cachedLineString.StartsWith("text{", StringComparison.OrdinalIgnoreCase));

                        if (!isSvgRelated && !isSvgElement)
                        {
                            if (IsEnabled())
                            {
                        ...
                        // If it's $svg, $end, or SVG element, fall through to normal processing
                    }
                    ...
                    // Check for $end if we're in SVG block mode
                    if (_isSvgBlock && textSpan.StartsWith("$end", StringComparison.OrdinalIgnoreCase))
                    {
                        try
                        {
                            var svgHtml = _svgParser.GenerateHtml(_calculate);
                            _sb.Append(InsertAttribute(svgHtml, HtmlId));
                        }
                    ...
                    // If in SVG block mode, check if line contains SVG element
                    // BUT allow control flow commands (#for, #next, #if, etc.) to pass through
                    // NOTE: We intercept SVG elements regardless of _isVisible so they can be processed
                    // even when wrapped in #hide...#show blocks
                    // IMPORTANT: Intercept ALWAYS when in SVG block (not just when _calculate)
                    // to prevent SVG elements from being parsed as expressions in the first pass
                    // Also add elements even when !_calculate so Input mode shows the SVG
                    if (_isSvgBlock)
                    {
                        var lineStr = textSpan.ToString().Trim();
                        // Only intercept SVG elements, not comments (') or commands (#)
                        if (!lineStr.StartsWith("#") && !lineStr.StartsWith("'") &&
                            lineStr.Contains('{') && (
                            lineStr.StartsWith("line{") || lineStr.StartsWith("rect{") ||
                            lineStr.StartsWith("circle{") || lineStr.StartsWith("ellipse{") ||
                            lineStr.StartsWith("polygon{") || lineStr.StartsWith("polyline{") ||
                            lineStr.StartsWith("path{") || lineStr.StartsWith("text{")))
                        {
                            // Always add to SVG parser so both Input and Output modes show the SVG
                            _svgParser.AddElementLine(lineStr);
                            continue;
                        }
                    }
            ...
            bool ParsePlot(ReadOnlySpan<char> s)
            {
                ...
                {
                    // For $svg, start a block mode REGARDLESS of _isVisible
                    // This allows SVG blocks to work inside #hide...#show blocks
                    if (s.StartsWith("$s", StringComparison.OrdinalIgnoreCase))
                    {
                        _svgParser = new SvgParser(_parser, Settings.Plot);
                        _svgParser.ParseSvgConfigLine(s.ToString());
                        _isSvgBlock = true;
                    ...
                    if (_isVisible && IsEnabled())
                    {
                        ...
                        scriptToPlot = s.ToString();

                        try
                        {
                            _parser.IsPlotting = true;
                            var s1 = plotParser.Parse(scriptToPlot, _calculate);
